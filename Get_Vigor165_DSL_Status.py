...
def hue_request(method, url, **kwargs):
    ...
    while True:
        try:
            ...
            if not error_logged:
                print("Hue request error, retrying: ", strftime("%Y-%m-%d %H:%M:%S", gmtime()), "-", str(e))
                sys.stdout.flush()
                error_logged = True
...
def lights_on(bri):
    hue_request("PUT", f"http://{HUE_BRIDGE_IP}/api/{API_KEY}/groups/{GROUP_NR}/action/", headers=HEADERS, data=f'{{"on": true, "bri": {bri}, "transitiontime": 0}}')

# This will switch the lights off
def lights_off():
    hue_request("PUT", f"http://{HUE_BRIDGE_IP}/api/{API_KEY}/groups/{GROUP_NR}/action/", headers=HEADERS, data=f'{{"on": false, "transitiontime": 0}}')
...
def set_colour(colour):
    ...
    hue_request("PUT", "http://" + HUE_BRIDGE_IP + "/api/"  + API_KEY + "/groups/" + GROUP_NR + "/action/", headers=HEADERS, data=colour)

# This will set the brightness to the value provided
# The colour will stay unchanged
def new_bri(value):
    hue_request("PUT", f"http://{HUE_BRIDGE_IP}/api/{API_KEY}/groups/{GROUP_NR}/action/", headers=HEADERS, data=f'{{"bri": {value}, "transitiontime": 0}}')

# If lights are off turn them on, if they are on turn them off
def toggle_lights():
    # Find out whether any of the ADSL lights are on
    dict = hue_request("GET", f"http://{HUE_BRIDGE_IP}/api/{API_KEY}/groups/{GROUP_NR}")
    if json.loads(dict.text)['state']['any_on']:
        hue_request("PUT", f"http://{HUE_BRIDGE_IP}/api/{API_KEY}/groups/{GROUP_NR}/action/", headers=HEADERS, data='{"on":false, "bri": 0, "transitiontime": 0}')
    else:
        hue_request("PUT", f"http://{HUE_BRIDGE_IP}/api/{API_KEY}/groups/{GROUP_NR}/action/", headers=HEADERS, data='{"on":true, "bri": 254, "transitiontime": 0}')
